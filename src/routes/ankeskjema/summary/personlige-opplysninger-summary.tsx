...
const PersonligeOpplysningerSummary = ({ user }: Props) => {
    ...
    return (
        <SpaceBetweenFlexListContainer>
            <InformationPointBox header={ankeskjema.summary.sections.person.given_name}>
                <Normaltekst>{combineFirstMiddleName(user)}</Normaltekst>
            </InformationPointBox>
            <InformationPointBox header={ankeskjema.summary.sections.person.surname}>
                <Normaltekst>{user.navn.etternavn ?? ''}</Normaltekst>
            </InformationPointBox>
            <InformationPointBox header={ankeskjema.summary.sections.person.nin}>
                <Normaltekst>
                    {foedselsnrFormat(user.folkeregisteridentifikator?.identifikasjonsnummer ?? '')}
                </Normaltekst>
            </InformationPointBox>
            <InformationPointBox header={ankeskjema.summary.sections.person.phone}>
                <Normaltekst>{user.kontaktinformasjon?.telefonnummer ?? ''}</Normaltekst>
            </InformationPointBox>
            <InformationPointBox header={ankeskjema.summary.sections.person.address}>
